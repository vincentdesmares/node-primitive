...
class CanvasWrapper {
  ...
  static original (url, cfg, document) {
    ...
    debugInternal('document', document)
    return new Promise((resolve, reject) => {
      debugInternal('Loading the image', url)
      try {
        let img = new document.defaultView.Image()
        img.onerror = debugInternal
        img.onload = e => {
          debugInternal('image loaded')
          debugInternal('document after loading', document)
          try {
            let w = img.width
            let h = img.height
            debugInternal('w/h', w, h)
            ...
            debugInternal('img', img, cfg.width, cfg.height)
            ...
            debugInternal('resolving canvas')
            resolve(canvas)
          } catch (e) {
            debugInternal('error resolving canvas', e.message, e.stack)
            reject(e.message)
          }
        }
        img.src = url
